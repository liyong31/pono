...
ProverResult IC3Base::check_until(int k)
{
  ...
  while (i <= k) {
    res = step(i); // check whether F(i) /\ T /\ !P
...
IC3Formula IC3Base::inductive_generalization(size_t i, const IC3Formula & c)
{
  ...
  while (j < gen.children.size() && gen.children.size() > 1) {
    ...
    if (!check_intersects_initial(gen.term)
      ...
      logger.log(3, "Can drop term: {}", dropped);
      ...
      logger.log(3, "Can not drop term: {}", dropped);
...
// check whether bad is reachable from current frame within one-step
bool IC3Base::reaches_bad(IC3Formula & out)
{
  ...
  logger.log(3, "reach next bad: {}", ts_.next(bad_));
  solver_->assert_formula(trans_label_);
  logger.log(3, "reach bad trans: {}", trans_label_);
  Result r = check_sat();

  if (r.is_sat()) {
    out = get_model_ic3formula(); // get a model in current frame
    for (Term tm : out.children)
    {
      logger.log(3, "reaches_bad model_ic3_formula: {}", tm);
    }
    assert(out.term);
    assert(out.children.size());
    assert(ic3formula_check_valid(out));

    if (options_.ic3_pregen_) {// by default this is true
...
ProverResult IC3Base::step(int i)
{
  ...
  if (reached_k_ < 1) {
    return step_01();//step 0 and 1, 0: init & bad, 1: init & T & bad_
  }
...
ProverResult IC3Base::step_01()
{
  ...
  if (r.is_sat()) {
    ...
    pop_solver_context();
    ProofGoal * pg = new ProofGoal(c, 0, nullptr);//prove c is not reached from init
...
// F(i-1) /\ !c /\ T /\ !(!c) is UNSAT, indicating c can be blocked
// i.e., F(i-1) /\ !c /\ T => !c (do not consider self-loops in c)
  ...
  if (r.is_sat()) {
    ...
    for (size_t i = 0; i < assumps_.size(); ++i) {
      if (core.find(assumps_.at(i)) == core.end()) {
        ...
        logger.log(3, "orig bool predicate unsat core: {}", c.children.at(i));
      } else {
        gen.push_back(c.children.at(i));
        logger.log(3, "newly bool predicate unsat core: {}", c.children.at(i));
      }
    }
...
// Helper methods
// blocking phase, first need to check whether bad is one-step reachable
...
bool IC3Base::is_blocked(const ProofGoal * pg)
{
  ...
  logger.log(3, "is_blocked current goal term: {}", pg->target.term);
  Result r = check_sat();
  pop_solver_context();
  logger.log(3, "is_blocked result: {}", r.is_unsat());
...
// generalize this prefix to proof goal?
void IC3Base::predecessor_generalization_and_fix(size_t i,
                                                 const Term & c,
                                                 IC3Formula & pred)
{
  logger.log(3, "pred_and_fix: {}, {}, {}", i, c, pred.term);
...
void IC3Base::push_frame()
{
  ...
  if (frames_.size() > 1) {
    ...
    logger.log(
            3, "Add push frame formula {}: {}", frames_.size() - 1, solver_->make_term(Implies, frame_labels_.back(), prop));
  }
  // else == 1
}
// add constraint to all frame j with j <= i
void IC3Base::constrain_frame(size_t i, const IC3Formula & constraint,
                              bool new_constraint)
{
  logger.log(3, "propogate learnt constraint: {}", constraint.term);
  assert(solver_context_ == 0);
  ...
  if (new_constraint) {
    for (size_t j = 1; j <= i; ++j) {
      ...
      for (size_t l = 0; l < Fj.size(); ++l) {
        logger.log(6, "Frame {}: {}", j, Fj[l].term);
  ...
  frames_.at(i).push_back(constraint); //added to current frame
}
...
void IC3Base::assert_frame_labels(size_t i) const
{
  ...
  for (size_t j = 0; j < frame_labels_.size(); ++j) {
    ...
    logger.log(3, "asset frame labels: {}", assump);
  }
}
...
size_t IC3Base::find_highest_frame(size_t i, IC3Formula & u)
{
  ...
  logger.log(3, "Find highest frame {} to constrain {}", j, u.term);
...
// add boolean symbol for a predicate
